...
function renderInfographics() {
    ...
    for (const section of infographicSections) {
        ...
        for (const img of p.images) {
            html += `<div class="infographic-item" data-src="${IMG}${encodeURIComponent(img.file)}"><img src="${IMG}${encodeURIComponent(img.file)}" alt="${img.title}" loading="lazy"><h3>${img.title}</h3></div>`;
        }
        html += `</div>`;
    }
    return html;
...
function renderSlideIndex() {
    ...
    for (const key of deckPages) {
        ...
        for (const deck of p.decks) {
            ...
            const icon = isVideo ? '&#9654;' : '&#128196;';
            html += `<a href="${PDF}${encodeURIComponent(deck.file)}" target="_blank" class="deck-item"><div class="deck-icon">${icon}</div><div class="deck-info"><h3>${deck.title}</h3><span class="deck-type">${deck.type}</span></div></a>`;
        }
    }
    html += `</div>`;
    return html;
